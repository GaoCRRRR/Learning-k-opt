...
from utils import AverageMeter, freeze, initialize_weights
...
# policy_1 = ActorCriticNetwork(args.input_dim,
#                               args.embedding_dim,
#                               args.hidden_dim,
#                               args.n_points,
#                               args.n_rnn_layers,
...
checkpoint = torch.load(
    os.path.join(os.getcwd(), args.load_path), map_location='cuda:'+str(args.gpu_n))
...
# initialize_weights(policy, policy_1.encoder)
# initialize_weights(policy, policy_1.encoder_star)

# del policy_1
# gc.collect()

freeze(policy.encoder)
freeze(policy.encoder_star)

# define the optimizer and scheduler
if args.rms_prop:
    optimizer = RMSprop(filter(lambda p: p.requires_grad, policy.parameters()), lr=args.lr, weight_decay=args.wd)
else:
    optimizer = Adam(filter(lambda p: p.requires_grad, policy.parameters()),
                     lr=args.lr,
