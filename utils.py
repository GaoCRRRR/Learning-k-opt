...
def initialize_weights(actor, encoder):
    pretrained_encoder = encoder.state_dict()
    all_params = {}
    for k, v in actor.encoder.state_dict().items():
        if k in pretrained_encoder.keys():
            v = pretrained_encoder[k]
            all_params[k] = v

    assert len(all_params.keys()) == len(actor.encoder.state_dict().keys())
    actor.encoder.load_state_dict(all_params)
...
def swap_2opt(tour, i, k):
    ...
    new_tour = tour[0: i]
    new_tour = np.append(new_tour, np.flip(tour[i: k], axis=0))
...
def swap_2opt_new(tour, i, k, tour_distance, D):
    ...
    remove = D[tour[pred_i]][tour[i]] + D[tour[pred_k]][tour[k]]
    ...
    add = D[new_tour[pred_i]][new_tour[i]] + D[new_tour[pred_k]][new_tour[k]]
    # print("add", add)
    distance += add
    # print("tour distance after 2 opt", distance)
    return new_tour, distance

def _3_opt(tour, first, second, third, tour_distance, D):
    if(first >= second - 1):
        return tour, tour_distance
    else:
        new_tour_, new_distance_ = swap_2opt_new(tour, first, second, tour_distance, D)

    if(second >= third - 1):
        return new_tour_, new_distance_
    else:
        new_tour, new_distance = swap_2opt_new(new_tour_, first, third, new_distance_, D)
        return new_tour, new_distance

def _4_opt(tour, first, second, third, fourth, tour_distance, D):
    if (first >= second - 1):
        return tour, tour_distance
    else:
        tour_1, distance_1 = swap_2opt_new(tour, first, second, tour_distance, D)
    if (second >= third - 1):
        return tour_1, distance_1
    else:
        tour_2, distance_2 = swap_2opt_new(tour_1, first, third, distance_1, D)
    if (third >= fourth - 1):
        return tour_2, distance_2
    else:
        tour_3, distance_3 = swap_2opt_new(tour_2, first, fourth, distance_2, D)
        return tour_3, distance_3
